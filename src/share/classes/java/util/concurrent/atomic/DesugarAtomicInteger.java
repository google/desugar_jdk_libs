 ...
 * @author Doug Lea
*/
// For desugar: Copy of j.u.c.a.AtomicInteger with methods added with 1.8
public class DesugarAtomicInteger {
    private DesugarAtomicInteger() {} // for desugar: no instantiation, static members only
     ...
     */
    // For desugar: made static so it can exist outside original class
    public static int accumulateAndGet(AtomicInteger atomic, int x,
                                       IntBinaryOperator accumulatorFunction) {
        int prev, next;
        do {
            prev = atomic.get();
