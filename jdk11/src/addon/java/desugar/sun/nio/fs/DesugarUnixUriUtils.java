...
class DesugarUnixUriUtils {
  private DesugarUnixUriUtils() {}

  /** Converts URI to Path */
  static DesugarUnixPath fromUri(FileSystem fs, URI uri, String userDir, String rootDir) {
        if (!uri.isAbsolute())
    ...
    // For desugar: Only suport file scheme.
    // TODO(deltazulu): Revisit this check once java.io.File#toPath becomes supported.
    ...
    return new DesugarUnixPath(
        /* fileSystem= */ fs,
        /* rawPath= */ new String(result, DesugarUtil.jnuEncoding()),
        userDir,
        rootDir);
    }

  /** Converts Path to URI */
  static URI toUri(DesugarUnixPath up) {
      ...
      // for desugar: Use java.io.File API to check if d
      // try {
      //     up.checkRead();
